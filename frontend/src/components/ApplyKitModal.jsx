    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 px-4 py-8 backdrop-blur-sm">
      <div className="max-h-[90vh] w-full max-w-4xl overflow-y-auto rounded-[2rem] border border-white/10 bg-slate-900 p-6 text-slate-100 shadow-2xl shadow-black/40">
        ...
        <div className="mt-6 rounded-[1.75rem] border border-white/10 bg-slate-950/60 p-6">
          ...
          {currentItem ? (
            currentItem.contentType === 'learning_path' ? (
              <div className="grid gap-4">
                {currentItem.content.map((item, index) => (
                  <article key={`${item.skill}-${index}`} className="rounded-3xl border border-white/10 bg-white/5 p-4">
                    <p className="text-sm font-medium text-cyan-200">{item.skill}</p>
                    <a href={item.resource_url} target="_blank" rel="noreferrer" className="mt-2 block text-sm text-cyan-300 underline underline-offset-4">
                      {item.resource_url}
                    </a>
                    <p className="mt-2 text-xs uppercase tracking-[0.2em] text-slate-400">{item.estimated_hours} estimated hours</p>
                  </article>
                ))}
              </div>
            ) : currentItem.contentType === 'interview_questions' ? (
